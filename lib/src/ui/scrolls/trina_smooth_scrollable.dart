...
class TrinaSmoothScrollableState extends State<Scrollable>
  ...
  void _updatePosition() {
    ...
    if (widget.physics != null) {
      ...
      _physics = widget.scrollBehavior!
  ...
  void _handleDragStart(DragStartDetails details) {
    ...
    assert(_drag == null);
    // Cancel any ongoing smooth scroll animation when user starts dragging
    _cancelPointerScroll();
  ...
  void _handlePointerScroll(PointerEvent event) {
    ...
    if (_pointerScrollTimer != null) {
      // If the timer is running, check if direction changed
      final double currentTarget = _accumulatedTargetOffset;
      final double currentPosition = position.pixels;

      // Check if the new delta would change direction
      final bool directionChanged =
          (currentTarget > currentPosition && delta < 0) ||
          (currentTarget < currentPosition && delta > 0);

      if (directionChanged) {
        // Direction changed - reset to current position and start fresh
        _accumulatedTargetOffset = (currentPosition + delta).clamp(
          minScrollExtent,
          maxScrollExtent,
        );
      } else {
        // Same direction - accumulate the delta
        _accumulatedTargetOffset += delta;
        _accumulatedTargetOffset = _accumulatedTargetOffset.clamp(
          minScrollExtent,
          maxScrollExtent,
        );
      }
      ...
      _pointerScrollTimer = Timer.periodic(const Duration(milliseconds: 16), (
        Timer timer,
      ) {
        ...
        if (difference.abs() < 2.0) {
          ...
          // Increment 30% of the remaining difference for faster response.
          final double increment = difference * 0.3;
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(
      DiagnosticsProperty<ScrollPhysics>('effective physics', _physics),
    );
  }
}
