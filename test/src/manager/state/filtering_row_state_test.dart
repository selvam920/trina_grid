...
void main() {
  ...
  setUp(() {
    columns = [...ColumnHelper.textColumn('column', count: 2, width: 150)];
  ...
  group('hasFilter', () {
    test('when filter is not set, '
        'should be returned false.', () {
      expect(stateManager.hasFilter, isFalse);
    });

    test('when filter is set, '
  ...
  group('setFilter', () {
    test('should be changed to none of state of row.', () {
      var filter = FilterHelper.convertRowsToFilter([
        FilterHelper.createFilterRow(),
      ], []);
    ...
    test('when filter is null, filterRows should be set to an empty List', () {
  ...
  group('filterRowsByField', () {
    test('When there is 0 column1 in filterRows, '
    ...
    test('When there is 1 column1 in filterRows, '
    ...
    test('When there is 2 column1 in filterRows, '
  ...
  group('isFilteredColumn', () {
    test('when there is no filter, should be returned false.', () {
    ...
    test('when filterRows is empty, should be returned false.', () {
      var column = stateManager.columns.first;
    ...
    test('when the column is filtered, should be returned true.', () {
      var column = stateManager.columns.first;
    ...
    test('when the column is not filtered, should be returned false.', () {
      var column = stateManager.columns.first;
  ...
  group('setColumnFilter', () {
    test('should add new filter when column has no existing filter', () {
      expect(stateManager.filterRows.length, 0);
    ...
    test('should update existing filter when column already has filter', () {
      // Add initial filter
      stateManager.setColumnFilter(
        columnField: 'column1',
        filterType: const TrinaFilterTypeContains(),
    ...
    test('should add multiple filters for different columns', () {
      stateManager.setColumnFilter(
        columnField: 'column1',
        filterType: const TrinaFilterTypeContains(),
        filterValue: 'test1',
      ...
      expect(
        column1Filter.cells[FilterHelper.filterFieldValue]!.value,
        'test1',
      );
      expect(
        column2Filter.cells[FilterHelper.filterFieldValue]!.value,
        'test2',
      );
    });
  });
  ...
  group('removeColumnFilter', () {
    test('should remove specific column filter', () {
    ...
    test('should do nothing when removing non-existent filter', () {
      stateManager.setColumnFilter(
        columnField: 'column1',
        filterType: const TrinaFilterTypeContains(),
        filterValue: 'test',
  ...
  group('clearAllColumnFilters', () {
    ...
    test('should do nothing when no filters exist', () {
      expect(stateManager.filterRows.length, 0);
  ...
  group('getColumnFilterValue', () {
    test('should return filter value for existing filter', () {
      stateManager.setColumnFilter(
        columnField: 'column1',
        filterType: const TrinaFilterTypeContains(),
    ...
    test('should return null for non-existent filter', () {
      var filterValue = stateManager.getColumnFilterValue('nonexistent');

      expect(filterValue, isNull);
    });

    test('should return correct values for different data types', () {
      // String value
      stateManager.setColumnFilter(
        columnField: 'column1',
        filterType: const TrinaFilterTypeContains(),
  ...
  group('getColumnFilterType', () {
    test('should return filter type for existing filter', () {
      stateManager.setColumnFilter(
        columnField: 'column1',
        filterType: const TrinaFilterTypeEquals(),
        filterValue: 'test',
    ...
    test('should return correct filter types for different filters', () {
      stateManager.setColumnFilter(
        columnField: 'column1',
        filterType: const TrinaFilterTypeContains(),
        filterValue: 'test',
      ...
      expect(
        stateManager.getColumnFilterType('column1'),
        isA<TrinaFilterTypeContains>(),
      );
      expect(
        stateManager.getColumnFilterType('column2'),
  ...
  group('integration tests', () {
    test('should handle complete filter lifecycle', () {
    ...
    test('should maintain filter integrity with mixed operations', () {
      // Add multiple filters
      stateManager.setColumnFilter(
        columnField: 'column1',
        filterType: const TrinaFilterTypeContains(),
