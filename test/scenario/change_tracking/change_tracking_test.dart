  ...
  group('Change Tracking Test', () {
    ...
    trinaGrid.test(
      ...
      (tester) async {
        ...
        expect(
          decoration.color,
          stateManager!.configuration.style.cellDirtyColor,
        );
      },
    );

    trinaGrid.test(
      'when cell value is changed, the `cell.isDirty` should be true',
      (tester) async {
    ...
    group('Pasting Into Table', () {
      trinaGrid.test(
        'after pasting text into a cell, `cell.isDirty` should be true',
        (tester) async {
          final cellFinder = find.byKey(cell.key).first;
          await tester.tap(cellFinder);
          await tester.pumpAndSettle();
  ...
  group('Committing changes', () {
    ...
    trinaGrid.test(
      ...
      (tester) async {
        ...
        expect(
          rows.first.cells.values.every(
            (cell) => !cell.isDirty && cell.value == 'New',
          ),
          true,
        );
      },
    ...
    trinaGrid.test(
      'after committing cell changes, `cell.oldValue` should be null',
      (tester) async {
        final cellFinder = find.byKey(cell.key).first;
        await tester.tap(cellFinder);
        await tester.pumpAndSettle();
